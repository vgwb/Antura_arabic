...
namespace EA4S.Rewards
{
    ...
    public class PlaySessionResultManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            int itemsToUnlock = AppManager.Instance.NavigationManager.CalculateUnlockItemCount();
            int earnedStars = AppManager.Instance.NavigationManager.CalculateStarsCount();

            List<RewardPackUnlockData> oldRewards = AppManager.Instance.Player.RewardsUnlocked.Where(ru => ru.GetJourneyPosition().Equals(AppManager.Instance.Player.CurrentJourneyPosition)).ToList();
            ...
            for (int i = 0; i < itemsToUnlock - itemAlreadyUnlocked; i++) {
                ...
                AppManager.Instance.Player.AddRewardUnlocked(newRewardToUnlock);
            }

            // Show UI result and unlock transform parent where show unlocked items
            GameObject[] objs = new GameObject[] { };
            objs = GameResultUI.ShowEndsessionResult(AppManager.Instance.NavigationManager.UseEndSessionResults(), itemAlreadyUnlocked);
            ...
            LogManager.I.LogPlaySessionScore(AppManager.Instance.JourneyHelper.GetCurrentPlaySessionData().Id, objs.Length);
            AppManager.Instance.Teacher.logAI.UnlockVocabularyDataForJourneyPosition(AppManager.Instance.Player.CurrentJourneyPosition);
