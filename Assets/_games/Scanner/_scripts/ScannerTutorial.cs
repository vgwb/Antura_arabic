    ...
    public class ScannerTutorial : MonoBehaviour
    {
            ...
            else if (step == 2)
            {
                foreach (ScannerSuitcase sc in gameManager.suitcases)
                    if (sc.isCorrectAnswer)
                    {
                        currentSuitcases = sc;
                        source = sc.transform;
                        target = gameManager.scannerLL[0].transform;
                        break;
                    }
            }
        ...
        IEnumerator coDoTutorial()
        {
            ...
            while (isTutRound)
            {
                ...
                if (step == 1)
                    TutorialUI.DrawLine(source.position - Vector3.forward * 2, gameManager.scannerLL[0].transform.position + 
						new Vector3(-2f, scannerDevice.position.y - gameManager.scannerLL[0].transform.position.y, -2), 
