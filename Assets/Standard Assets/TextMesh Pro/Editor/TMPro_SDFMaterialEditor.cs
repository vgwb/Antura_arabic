...
public class TMPro_SDFMaterialEditor : MaterialEditor
{
    ...
    public override void OnInspectorGUI()
    {
            ...
            if (m_foldout.debug)
            {
                ...
                if (targetMaterial.HasProperty("_ClipRect"))
                {
                    ...
                    if (m_mask != MaskingTypes.MaskOff)
                    {
                        ...
                        if (targetMaterial.HasProperty("_MaskEdgeColor"))
                        {
                            DrawTextureProperty(m_maskTex, "Mask Texture");

                            bool isInverted = m_maskTexInverse.floatValue == 0 ? false : true;
                            isInverted = EditorGUILayout.Toggle("Inverse Mask", isInverted);
                            ...
                            if (EditorGUI.EndChangeCheck())
                            {
                                m_maskTexInverse.floatValue = isInverted ? 1 : 0;
                                havePropertiesChanged = true;
                            }
                        }
        ...
        // Check to make sure target material matches the assigned material
        //if ((m_textComponent as TextMeshPro) != null)
        //{
        //    if (targetMaterial != m_textComponent.fontSharedMaterial)
        //    {
        //        //TMPro_EventManager.ON_DRAG_AND_DROP_MATERIAL_CHANGED(Selection.activeGameObject, m_textComponent.fontSharedMaterial, targetMaterial);
        //        m_textComponent.fontSharedMaterial = targetMaterial;
        //        TMPro_EditorUtility.RepaintAll();
        //    }
        //}
    }
        ...
        switch (evt.type)
        {
            ...
            case EventType.DragPerform:
                ...
                if (evt.type == EventType.DragPerform)
                {
                    ...
                    Texture currentMainTex = currentMaterial.GetTexture(ShaderUtilities.ID_MainTex);
                    ...
                    if (newMaterial == null || newMaterial == currentMaterial || newMaterial == null || newMainTex == null)
                        break;

                    // Check to make sure the new material's font atlas texture matches the currently assigned material
                    if (newMainTex.GetInstanceID() != currentMainTex.GetInstanceID())
                    {
    ...
    private void DrawSliderProperty(MaterialProperty property, string label)
    {
        ...
        FloatProperty(new Rect(pos1), property, string.Empty);
    ...
    private void Draw2DRectBoundsProperty(MaterialProperty property, string label)
    {
        ...
        float width = (pos0.width - pos0.x - 30) / 4;
        pos0.x += old_LabelWidth - 30;
        ...
        pos0.width = width + 8;
        vec.x = EditorGUI.FloatField(pos0, "BL", vec.x);

        pos0.x += width;
        pos0.width = width - 18;
        vec.y = EditorGUI.FloatField(pos0, "", vec.y);

        pos0.x += width - 24;
        pos0.width = width + 8;
        vec.z = EditorGUI.FloatField(pos0, "TR", vec.z);

        pos0.x += width;
        pos0.width = width - 18;
        vec.w = EditorGUI.FloatField(pos0, "", vec.w);
    ...
    void ReadMaterialProperties()
    {
        ...
        m_maskTexInverse = GetMaterialProperty(target_Materials, "_MaskInverse");
        m_maskTexEdgeColor = GetMaterialProperty(target_Materials, "_MaskEdgeColor");
        m_maskTexEdgeSoftness = GetMaterialProperty(target_Materials, "_MaskEdgeSoftness");
        m_maskTexWipeControl = GetMaterialProperty(target_Materials, "_MaskWipeControl");
